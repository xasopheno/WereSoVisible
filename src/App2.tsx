...
export default class App extends React.Component {
  ...
  public componentDidMount() {
    ...
    this.scene.fog = new THREE.Fog(0x050505, 1000, 3500);
    ...
    for (let i = 0; i < data.length; i++) {
      if (data[i].z > 0.0 && data[i].y > 20.0 && data[i].event_type === 'On') {
        // positions
        const x = data[i].x * 150.0;
        const y = Math.log(data[i].y) * 100 - 600;
        const z = data[i].t * 20;
